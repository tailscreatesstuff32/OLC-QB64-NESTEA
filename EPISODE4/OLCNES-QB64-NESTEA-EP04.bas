...
DIM SHARED AS _OFFSET hMenuBar, hSubMenu
...
SUB main ()
    ...
    DO
        ...
        IF keypress(ASC("'")) THEN scanline = 241: cycle = 1

        '  IF cpu_regs.pc = &H8057~% THEN bEmulationrun = 0
        '       IF cpu_regs.pc = &H8E11~% THEN bEmulationrun = 0

        ' IF cpu_regs.pc = &H8E09~% THEN bEmulationrun = 0
        ' IF cpu_regs.pc = &H8213~% THEN bEmulationrun = 0


        '  IF cpu_regs.pc = &H8EB8~% THEN bEmulationrun = 0
        '
        '   IF cpu_regs.pc = &HC007~% THEN bEmulationrun = 0
        ...
        IF bEmulationrun THEN 'wip for now.... slowish
            ...
            DO
                ...
                clock_NES
                '   IF cpuRam(&HE9) = &H9D THEN EXIT DO
                '  IF cpu_regs.pc = &H9C19~% THEN EXIT DO


                '   IF tblPalette(0) > 0 AND tblPalette(0) <> &H07 THEN bEmulationrun = 0: EXIT DO

                '  IF cpu_regs.pc = &HC3C6~% THEN EXIT DO
                '  IF cpu_regs.pc = &HFCF7~% THEN EXIT DO: bEmulationrun = 0
                '   IF cpuRam(&H02) = &H68 THEN EXIT DO: bEmulationrun = 0

                '  IF cpu_regs.pc = &H8508~% THEN EXIT DO: bEmulationrun = 0

                '  IF cpuRam(&H15) = &H30~% THEN bEmulationrun = 0: EXIT DO


                'IF instructions(bus_cpuRead(cpu_regs.pc, 1)).op_name = "ROL" THEN
                '    bEmulationrun = 0
                '    EXIT DO:
                'END IF


                'IF cpu_regs.pc = &HC474~% THEN
                '    bEmulationrun = 0
                '    EXIT DO:
                'END IF
                ...
                'IF cpu_regs.pc = &H893A~% THEN
                '    bEmulationrun = 0
                '    EXIT DO:
                'END IF


                'IF tblPalette(0) > &H0 THEN bEmulationrun = 0: EXIT DO

                'IF cpu_regs.pc = &H8610~% THEN
                '    bEmulationrun = 0
                '    EXIT DO:
                'END IF


                'IF cpuRam(&H304) = &H22 THEN bEmulationrun = 0: EXIT DO



                '      IF tblPalette(0) = &H17 THEN bEmulationrun = 0: EXIT DO
                ...
                'IF cpu_regs.pc = &H8508~% THEN
                '    bEmulationrun = 0
                '    EXIT DO:
                'END IF


                'IF cpu_regs.pc = &H83AD~% THEN
                '    bEmulationrun = 0
                '    EXIT DO:
                'END IF

                'IF cpu_regs.pc = &HCAC6~% THEN
                '    bEmulationrun = 0
                '    EXIT DO:
                'END IF

                'IF cpuRam(0) = &H2C~% THEN bEmulationrun = 0: EXIT DO


                'IF cpu_regs.y_reg = &H4F THEN
                '    bEmulationrun = 0
                '    EXIT DO:
                'END IF


                'IF cpu_regs.pc = &HCAC9~% THEN
                '    bEmulationrun = 0
                '    EXIT DO:
                'END IF


                'IF cpu_regs.x_reg = &HC1 THEN
                '    bEmulationrun = 0
                '    EXIT DO:
                'END IF
            ...
            framecomplete = 0
            '  IF cpuRam(&HE9) = &H9D THEN bEmulationrun = 0
            ' IF cpu_regs.pc = &H9C19~% THEN bEmulationrun = 0
            '      IF cpu_regs.pc = &HC3C6~% THEN bEmulationrun = 0


            '  IF tblPalette(0) <> 0 THEN bEmulationrun = 0


            'IF bEmulationrun THEN 'wip for now....need to test more game roms besides nestest.nes
